...
### **Resposta 1: O que é polimorfismo neste projeto?**

```
"Polimorfismo é quando diferentes objetos respondem ao mesmo comando
de formas diferentes. 

Neste projeto:
- Cada processador tem validate() e ingest()
- Mas cada um implementa de forma diferente
- O NumericProcessor valida números
- O TextProcessor valida strings
- O LogProcessor valida dicionários

O DataStream chama o mesmo código (proc.validate()),
mas cada processador responde diferente. Isso é polimorfismo!"
```

### **Resposta 2: Por que usar classes abstratas?**

```
"Classes abstratas servem como template/contrato.

Garantem que:
1. Todos os processadores implementem validate() e ingest()
2. Se tentar criar ProcessadorQualquer sem implementar, Python erro
3. O código fica seguro e previsível
4. Fácil adicionar novos tipos depois"
```

### **Resposta 3: Como o roteamento polimórfico funciona?**

```
"No Exercise 1, o DataStream recebe dados mistos [42, "hello", [1,2,3]]:

Para cada element:
  Para cada processador registrado:
    Se processador.validate(element) == True:
      processador.ingest(element)
      break  # Vai pro próxim element

Nenhum 'if' checando tipos específicos!
O polimorfismo escolhe automaticamente qual processador usar."
```

### **Resposta 4: O que é Protocol/Duck Typing?**

```
"Protocol define uma 'interface sem herança'.

No Exercise 2:
- Definou: class ExportPlugin(Protocol) com method process_output()
- CSV e JSON não herdam de ExportPlugin
- Mas têm o método process_output()
- DataStream aceita qualquer plugin que tenha este método

'Se quack como um pato, é um pato!'
Não precisa da herança, só da interface correta."
```

### **Resposta 5: Por que cada exercise foi separado?**

```
Exercise 0: Foundation
  - ABC e subclasses
  - Cada processador independente
  
Exercise 1: Roteamento inteligente
  - Polimorfismo em ação
  - DataStream orquestra tudo
  - Um stream de dados mistos
  
Exercise 2: Extensibilidade
  - Plugins podem ser adicionados sem mexer no DataStream
  - Protocol para contratos flexíveis
...
class MeuProcessador(DataProcessor):
    def validate(self, data):  # Só implementa validate
        pass
    # Falta ingest()!
...
### ❌ Erro 3: Type hints errados
```python
# ERRADO:
def ingest(self, data):  # Sem type hint
    pass
...
### ❌ Erro 4: Esquecer de converter para string no storage
```python
# ERRADO:
self._storage.append((self._next_rank, item))  # item é int/float

# CORRETO:
self._storage.append((self._next_rank, str(item)))  # converteu
```
...
Para praticar mais:
1. Crie um `DateProcessor` que valida datas
2. Crie um `XMLExportPlugin`
3. Adicione tratamento de erros mais robusto
4. Use `mypy` para validar tipos: `mypy module05/`
5. Use `flake8` para linting: `flake8 module05/`
